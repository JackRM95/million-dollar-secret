...
const KEY_MISSIONS       = "mds-missions-shared";
...
function autoExpireMissions(game) {
  ...
  players.forEach(p => {
    // Also resolve any guesses where targetMissionId was unknown at submit time
    // (target never responded, so todayMissions was never written for them)
    // We can still resolve these by computing their mission deterministically
    Object.keys(updated.guesses || {}).forEach(gk => {
      ...
      // Resolve: compute what mission target had that day
    ...
    Object.keys(updated.todayMissions || {}).forEach(key => {
      ...
      if (!updated.completions?.[ck] && !updated.notCompleted?.[ck]) {
        ...
        // Resolve guesses for this expired mission (target never completed)
      ...
      if (updated.completions?.[ck] && !updated.verifications?.[`${p.name}:${date}:${missionId}:awarded`]) {
        ...
        // Check if an exact guess was correct — if so, fail the mission instead of awarding
        const hasExactCorrectGuess = Object.keys(updated.guesses || {}).some(gk => {
          ...
          const guess = updated.guesses[gk];
          return guess.type === "exact" && guess.missionId === missionId;
        });

        if (hasExactCorrectGuess) {
          // Flip to not completed, increase difficulty — guesser already awarded at submit time
          ...
          // Normal award with category penalty if applicable
...
// GUESS MODAL
// ─────────────────────────────────────────────────────────────
function GuessModal({ guesser, target, game, today, onSubmit, onClose }) {
...
function PlayerView({ player, game, onSaveGame, onSwitchPlayer }) {
  ...
  const todayScores = allPlayers.map(p => {
    ...
    return { ...p, done, skip, pm, pck, pmId, iVerified, fullyVerified };
  });
  ...
  async function execRespond(type) {
    ...
    const histEntry = {
      ...
      completed,
      pts:       completed ? (DIFF_SCORE[mission?.difficulty] || 1) : 0,
    };
    ...
    Object.keys(updated.guesses || {}).forEach(gk => {
      ...
      if (guess.type === "exact" && guess.missionId === mId) {
        ...
        const missionPts = DIFF_SCORE[mission?.difficulty] || 1; // used only for difficulty logic
        const newCompletions = { ...updated.completions };
        delete newCompletions[ck];
        updated.completions  = newCompletions;
        updated.notCompleted = { ...updated.notCompleted, [ck]: true };
  ...
  async function submitGuess(targetPlayer, guessType, guessValue, guessMissionId) {
    ...
    let updated = {
      ...game,
      guesses:  { ...game.guesses,  [gk]: entry },
      balances: { ...game.balances, [player.name]: Math.max(0, (game.balances?.[player.name] || 0) - 1) },
    };

    const targetMissionId  = game.todayMissions?.[`${targetPlayer.name}:${today}`];
    const targetCk         = `${targetPlayer.name}:${today}:${targetMissionId}`;
    const targetMission    = targetMissionId ? game.missions.find(m => m.id === targetMissionId) : null;
    const alreadyCompleted = targetMissionId && !!game.completions?.[targetCk];
    const alreadyAwarded   = targetMissionId && !!game.verifications?.[`${targetPlayer.name}:${today}:${targetMissionId}:awarded`];
    ...
    const isCorrect         = isExactCorrect || isCategoryCorrect;

    // ── EXACT CORRECT: fail the mission for target, award guesser ──
    if (isExactCorrect && targetMissionId) {
      const completionRecord = game.completions?.[targetCk];
      const rawPts = (typeof completionRecord === "object" ? completionRecord?.pts : null)
                     || DIFF_SCORE[targetMission?.difficulty] || 1;
...
function PlayersTab({ game, onSave }) {
  ...
  async function resetMission(player) {
    ...
    await onSave({ ...g, completions:co, notCompleted:nc, missionOverrides:overrides });
    setConfirm(null);
  }
...
function ScoreboardTab({ game, onSave }) {
  ...
  async function reset() {
    setSaving(true);
    const latest = gameRef.current;
    await onSave({ ...latest, completions:{}, notCompleted:{}, ratings:{}, balances:{}, scores:{}, verifications:{}, exclusions:{}, todayMissions:{} });
    setSaving(false);
    setConfirm(false);
  }
...
export default function App() {
  ...
  const firstLoad   = useRef(true); // only set mode from subscribeShared on first snapshot

  const savingExpire = useRef(false); // prevent re-entrant autoExpire saves
  ...
  useEffect(() => {
    ...
    // On first load, pull shared missions and merge in
