...
function applyAdaptiveLogic(game, missionId, completed) {
  ...
  const missions = game.missions.map(m => {
    ...
    console.log(`[MDS] Difficulty: mission ${missionId} ${m.difficulty} -> ${newDiff} (completed=${completed})`);
    return { ...m, difficulty: newDiff };
  });
  if (!missions.some(m => m.id === missionId)) {
    console.warn(`[MDS] applyAdaptiveLogic: missionId ${missionId} not found in pool`);
  }
