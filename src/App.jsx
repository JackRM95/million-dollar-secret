...
function PlayerView({ player, game, onSaveGame, onSwitchPlayer }) {
  ...
  async function execRespond(type) {
    ...
    if (completed) {
      // For completions: record it, but do NOT adjust difficulty yet —
      // difficulty drops after verification so points are awarded at correct level
  ...
  async function verifyPlayer(targetPlayer, pmId) {
    ...
    if (!alreadyAwarded) {
      // Award points at current difficulty BEFORE dropping it
      ...
      // Now drop difficulty after points have been calculated
