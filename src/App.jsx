  ...
  players.forEach(p => {
    Object.keys(updated.todayMissions || {}).forEach(key => {
      ...
      if (updated.completions?.[ck] && !updated.verifications?.[`${p.name}:${date}:${missionId}:awarded`]) {
        ...
        // Check for correct guesses that apply penalties
        ...
        // Mark all guesses for this player/date as awarded
...
function PlayerView({ player, game, onSaveGame, onSwitchPlayer }) {
  ...
  async function submitGuess(targetPlayer, guessType, guessValue, guessMissionId) {
    ...
    const currentBalance = game.balances?.[player.name] || 0;
    ...
    const updated = {
      ...
      balances: { ...game.balances, [player.name]: Math.max(0, currentBalance - 1) },
    };
    await onSaveGame(updated);
    setGuessTarget(null);
  }
