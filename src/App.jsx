...
const GUESS_CAT_PTS      = 1;
const GUESS_EXACT_PTS    = 3;
...
function PlayerView({ player, game, onSaveGame, onSwitchPlayer }) {
  ...
  async function execRespond(type) {
    ...
    Object.keys(updated.guesses || {}).forEach(gk => {
      ...
      if (guess.type === "exact" && guess.missionId === mId) {
        ...
        const rawPts = DIFF_SCORE[mission?.difficulty] || 1;
        const newCompletions = { ...updated.completions };
        delete newCompletions[ck];
        updated.completions  = newCompletions;
        updated.notCompleted = { ...updated.notCompleted, [ck]: true };
        ...
        updated = applyAdaptiveLogic(updated, mId, false);
        // Award guesser score + balance
        updated.scores   = { ...updated.scores,   [guesserName]: (updated.scores?.[guesserName]   || 0) + rawPts };
        updated.balances = { ...updated.balances,  [guesserName]: (updated.balances?.[guesserName] || 0) + rawPts };
  ...
  async function submitGuess(targetPlayer, guessType, guessValue, guessMissionId) {
    ...
    if (isExactCorrect && targetMissionId) {
      ...
      // Award guesser: target's mission pts value → both score and balance
      updated.scores   = { ...updated.scores,   [player.name]: (updated.scores?.[player.name]   || 0) + rawPts };
      updated.balances = { ...updated.balances,  [player.name]: (updated.balances?.[player.name] || 0) + rawPts };
