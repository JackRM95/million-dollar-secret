...
function awardGuessPoints(game, targetName, dateStamp, missionId, targetMission) {
  ...
  Object.keys(updated.guesses || {}).forEach(gk => {
    // Key format: "guesserName:date:targetName"
    ...
    updated.guesses = {
      ...updated.guesses,
      [gk]: { ...guess, correct, awarded: true },
    };
    if (correct && earnedPts > 0) {
      updated.balances = {
        ...updated.balances,
        [guesserName]: (updated.balances?.[guesserName] || 0) + earnedPts,
      };
    }
  });
  return updated;
...
function PlayerView({ player, game, onSaveGame, onSwitchPlayer }) {
  ...
  const alreadyDone = !!game.completions?.[ck];  // truthy for both true and {pts:n}
  ...
  const [guessTarget,   setGuessTarget]   = useState(null); // player object to guess for
  ...
  async function execRespond(type) {
    ...
    if (completed) {
      // Store the points value at completion time so display is correct after difficulty drops
      const pts = DIFF_SCORE[mission?.difficulty] || 1;
      updated.completions = { ...updated.completions, [ck]: { pts } };
      setStatus("done");
    } else {
      // For failures: adjust difficulty immediately
      updated.notCompleted = { ...updated.notCompleted, [ck]: true };
      setStatus("skipped");
  ...
  async function verifyPlayer(targetPlayer, pmId) {
    ...
    if (!alreadyAwarded) {
      // Read pts from the completion record (stored at completion time, before difficulty drop)
      ...
      // Drop difficulty after points calculated
  ...
  async function submitGuess(targetPlayer, guessType, guessValue, guessMissionId) {
    ...
    if (game.guesses?.[gk]) return; // already guessed
    const entry = {
      type:      guessType,        // "category" | "exact"
      guess:     guessValue,       // category string or mission text
      missionId: guessMissionId || null, // mission id for exact guesses
      awarded:   false,
      correct:   null,
    };
      ...
      <PageWrap narrow>
            ...
            {todayScores.map(p => {
              ...
              return (
                <div key={p.id} style={{ background:C.dark2, border:`1px solid rgba(201,168,76,0.1)`, padding:"10px 14px", marginBottom:6 }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:(!isMe && (p.done || !p.skip)) ? 6 : 0 }}>
                  ...
                  {!isMe && (
                    <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                      ...
                      {/* Guess button — available until verified */}
                      {!game.verifications?.[`${p.name}:${today}:${p.pmId}:awarded`] && (
                        myGuess ? (
                          <span style={{ fontFamily:"monospace", fontSize:9, color:myGuess.awarded?(myGuess.correct?"#6ab47a":"#e05a4b"):C.goldDim, letterSpacing:"0.12em", textTransform:"uppercase" }}>
                            {myGuess.awarded ? (myGuess.correct ? `🕵 +${myGuess.type==="exact"?GUESS_EXACT_PTS:GUESS_CAT_PTS}pts correct!` : "🕵 Wrong guess") : `🕵 ${myGuess.type==="category"?"Cat:":"Exact:"} ${myGuess.guess.length>18?myGuess.guess.slice(0,18)+"…":myGuess.guess}`}
                          </span>
                        ) : (
        ...
        <div style={{ marginTop:28 }}>
          <button onClick={() => setShowHistory(h => !h)}
            style={{ background:"transparent", border:`1px solid rgba(201,168,76,0.2)`, fontFamily:"monospace", fontSize:9, letterSpacing:"0.35em", textTransform:"uppercase", color:C.gold, cursor:"pointer", padding:"8px 18px", width:"100%", textAlign:"left", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <span>📋 Mission History</span>
            <span style={{ opacity:0.6 }}>{showHistory ? "▲" : "▼"}</span>
          </button>
          {showHistory && (
            <div style={{ border:`1px solid rgba(201,168,76,0.15)`, borderTop:"none", padding:"12px 14px", background:C.dark2 }}>
...
function MissionHistoryPanel({ game, viewerName, allPlayers }) {
  ...
  function historyFor(name) {
    return (game.missionHistory?.[name] || []);
  }

  const hist = historyFor(activeTab);
  ...
  return (
    <div>
      {/* Player tabs */}
      {allPlayers.length > 1 && (
        <div style={{ display:"flex", gap:0, marginBottom:12, overflowX:"auto", borderBottom:`1px solid rgba(201,168,76,0.15)` }}>
          {tabs.map(name => (
            <button key={name} onClick={() => setActiveTab(name)}
              ...
              {name}{name===viewerName?" (you)":""}
            </button>
          ))}
        </div>
      )}
      {hist.length === 0 ? (
        <p style={{ fontFamily:"monospace", fontSize:10, color:C.creamDm, lineHeight:1.7 }}>
...
function GuessModal({ guesser, target, game, today, onSubmit, onClose }) {
  ...
  const categories = [...new Set((game.missions || []).map(m => m.category))].sort();
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(11,10,8,0.95)", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      <div style={{ background:C.dark2, border:`1px solid rgba(160,80,200,0.45)`, padding:"24px 20px", width:"100%", maxWidth:380, position:"relative", maxHeight:"80vh", overflowY:"auto" }}>
        ...
        <p style={{ fontFamily:"monospace", fontSize:9, letterSpacing:"0.4em", textTransform:"uppercase", color:"#c070e0", marginBottom:6 }}>🕵 Guess Mission</p>
        <p style={{ fontFamily:"'Georgia',serif", fontSize:15, color:C.cream, marginBottom:4 }}>What is {target.name} up to?</p>
        <p style={{ fontFamily:"monospace", fontSize:10, color:C.creamDm, marginBottom:18, lineHeight:1.6 }}>
          Category guess: +{GUESS_CAT_PTS}pt · Exact mission: +{GUESS_EXACT_PTS}pts
        </p>

        {/* Type toggle */}
        <div style={{ display:"flex", gap:0, marginBottom:16, border:`1px solid rgba(160,80,200,0.3)` }}>
          {[["category","Category"],["exact","Exact Mission"]].map(([val,label]) => (
            <button key={val} onClick={() => setGuessType(val)}
              style={{ flex:1, fontFamily:"monospace", fontSize:9, letterSpacing:"0.15em", textTransform:"uppercase", padding:"8px 0", cursor:"pointer", background:guessType===val?"rgba(160,80,200,0.25)":"transparent", color:guessType===val?"#c070e0":C.creamDm, border:"none", transition:"all 0.15s" }}>
              {label}
            </button>
          ))}
        </div>
        ...
        {guessType === "category" ? (
          <div>
            <p style={{ fontFamily:"monospace", fontSize:9, color:C.creamDm, letterSpacing:"0.2em", textTransform:"uppercase", marginBottom:8 }}>Select a category</p>
            <div style={{ display:"grid", gap:6, maxHeight:240, overflowY:"auto" }}>
              {categories.map(cat => (
                <button key={cat} onClick={() => setCatGuess(cat)}
                  style={{ fontFamily:"monospace", fontSize:10, letterSpacing:"0.1em", padding:"8px 12px", cursor:"pointer", textAlign:"left", background:catGuess===cat?"rgba(160,80,200,0.2)":"transparent", color:catGuess===cat?"#c070e0":C.cream, border:`1px solid ${catGuess===cat?"rgba(160,80,200,0.5)":"rgba(201,168,76,0.15)"}`, transition:"all 0.15s" }}>
                  {cat}
                </button>
              ))}
            </div>
