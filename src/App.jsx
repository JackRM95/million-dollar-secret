...
if (allDone)  missions = missions.map(m => m.id === missionId ? { …m, difficulty: diffDown(m.difficulty) } : m);
if (noneDone) missions = missions.map(m => m.id === missionId ? { …m, difficulty: diffUp(m.difficulty)   } : m);
...
const pm   = missionForPlayer(p.name, today, game.missions, game.missionOverrides, game.exclusions?.[p.name] || []);
const pck  = `${p.name}:${today}:${pm?.id}`;
const done = !!game.completions?.[pck];
const skip = !!game.notCompleted?.[pck];
const verifCount = Object.keys(game.verifications || {}).filter(k => k === `${player.name}:${p.name}:${today}:${pm?.id}`).length;
const iVerified  = !!game.verifications?.[`${player.name}:${p.name}:${today}:${pm?.id}`];
return { …p, done, skip, pm, pck, verifCount, iVerified };
});
...
? `Confirm you completed today's mission? You'll earn ${pts} point${pts !== 1 ? "s" : ""}.`
...
async function verifyPlayer(targetPlayer, targetCk) {
const missionId = targetCk.split(”:”)[2];
const vk = `${player.name}:${targetPlayer.name}:${today}:${missionId}`;
const alreadyVerified = Object.keys(game.verifications || {}).some(
k => k.startsWith(`${targetPlayer.name}:${today}:${missionId}:awarded`)
);
...
const awardKey = `${targetPlayer.name}:${today}:${missionId}:awarded`;
              ...
              {!isMe && p.done && (
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  {p.iVerified ? (
                    ...
                    <button onClick={() => verifyPlayer(p, p.pck)}
...
useEffect(() => {
Promise.all([loadShared(), loadIdentity()]).then(([g, ident]) => {
const game = migrateGame(g || defaultGame());
setGame(game);
