    ...
    Object.keys(updated.todayMissions || {}).forEach(key => {
      ...
      if (updated.completions?.[ck] && !updated.verifications?.[`${p.name}:${date}:${missionId}:awarded`]) {
        ...
        const hasExactCorrectGuess = Object.keys(updated.guesses || {}).some(gk => {
          ...
          const gDate = parts[1], gTarget = parts.slice(2).join(":");
...
function ScoreboardTab({ game, onSave }) {
  ...
  async function saveEdit(playerName) {
    ...
    await onSave({
      ...game,
      scores:   { ...game.scores,   [playerName]: newScore   },
      balances: { ...game.balances, [playerName]: newBalance },
    });
    setSaving(false);
    setEditing(null);
  }
  ...
  async function reset() {
    setSaving(true);
    await onSave({ ...game, completions:{}, notCompleted:{}, ratings:{}, balances:{}, scores:{}, verifications:{}, exclusions:{}, todayMissions:{} });
    setSaving(false);
    setConfirm(false);
  }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // (handles case where missions were updated via back office)
