...
const BAD_VOTE_THRESHOLD = 2;
const IS_STAGING         = import.meta.env.VITE_ENV === "staging";
const GUESS_CAT_PTS      = 2;
const GUESS_EXACT_PTS    = 4;
...
// ─────────────────────────────────────────────────────────────
// DEFAULT MISSIONS — with {otherPlayer} template token
// ─────────────────────────────────────────────────────────────
...
// Resolve {otherPlayer} token deterministically
...
// ─────────────────────────────────────────────────────────────
// SHARED MISSIONS (Firestore separate key)
// ─────────────────────────────────────────────────────────────
async function loadMissions() {
  try {
    const { doc, getDoc } = await import("firebase/firestore");
    const { db } = await import("./lib/firebase.js");
    const snap = await getDoc(doc(db, "game", "missions"));
    return snap.exists() ? snap.data().missions : null;
  } catch (e) {
...
async function saveMissions(missions) {
  try {
    ...
    const { db } = await import("./lib/firebase.js");
    await setDoc(doc(db, "game", "missions"), { missions });
    return true;
  } catch (e) {
    console.error("saveMissions error:", e);
    return false;
  }
}
...
function applyAdaptiveLogic(game, missionId, completed) {
  ...
  const missions = game.missions.map(m => {
    ...
    return { ...m, difficulty: completed ? diffDown(m.difficulty) : diffUp(m.difficulty) };
  });
  return { ...game, missions };
}

// Auto-expire missions from previous days that were never responded to
function autoExpireMissions(game) {
  ...
  // Resolve any unawarded guesses for previous days where target never locked todayMissions
  // (target never responded, so we compute their mission deterministically)
  const resolvedGuessKeys = new Set(); // track which guess keys we've already resolved
  players.forEach(p => {
    Object.keys(updated.guesses || {}).forEach(gk => {
      if (resolvedGuessKeys.has(gk)) return;
      const parts = gk.split(":");
      if (parts.length < 3) return;
      const gDate   = parseInt(parts[1], 10);
      const gTarget = parts.slice(2).join(":");
      if (gDate >= today || gTarget !== p.name) return;
      const guess = updated.guesses[gk];
      if (guess.awarded) return;
      const resolvedMission = missionForPlayerDeterministic(p.name, gDate, updated.missions, updated.exclusions?.[p.name] || []);
      if (!resolvedMission) return;
      let correct = false, earnedPts = 0;
      if (guess.type === "category" && resolvedMission.category === guess.guess) {
        correct = true; earnedPts = GUESS_CAT_PTS;
      } else if (guess.type === "exact" && guess.missionId === resolvedMission.id) {
        correct = true; earnedPts = GUESS_EXACT_PTS;
      }
      updated.guesses = { ...updated.guesses, [gk]: { ...guess, correct, awarded: true } };
      if (correct && earnedPts > 0) {
        const guesserName = parts[0];
        updated.scores   = { ...updated.scores,   [guesserName]: (updated.scores?.[guesserName]   || 0) + earnedPts };
        updated.balances = { ...updated.balances,  [guesserName]: (updated.balances?.[guesserName] || 0) + earnedPts };
      }
      resolvedGuessKeys.add(gk);
      changed = true;
    });
  });
  // Process todayMissions: expire unanswered, auto-award completed
  players.forEach(p => {
    Object.keys(updated.todayMissions || {}).forEach(key => {
      ...
      const targetMission = updated.missions.find(m => m.id === missionId);

      // Expire missions never responded to — then resolve any pending guesses against them
      if (!updated.completions?.[ck] && !updated.notCompleted?.[ck]) {
        ...
        updated = resolveGuessResults(updated, p.name, String(date), missionId, targetMission);
        changed = true;
      }

      // Auto-award points for completed-but-unverified missions from previous days
      if (updated.completions?.[ck] && !updated.verifications?.[`${p.name}:${date}:${missionId}:awarded`]) {
        const completionRecord = updated.completions[ck];
        const rawPts = (typeof completionRecord === "object" ? completionRecord?.pts : null)
                       || DIFF_SCORE[targetMission?.difficulty] || 1;

        const hasExactCorrectGuess = Object.keys(updated.guesses || {}).some(gk => {
          const gParts = gk.split(":");
          if (gParts.length < 3) return false;
          const gDate = gParts[1], gTarget = gParts.slice(2).join(":");
          if (gDate !== String(date) || gTarget !== p.name) return false;
          const guess = updated.guesses[gk];
          return guess.type === "exact" && guess.missionId === missionId && guess.correct !== false;
        });

        if (hasExactCorrectGuess) {
          const newCompletions = { ...updated.completions };
          delete newCompletions[ck];
          updated.completions  = newCompletions;
          updated.notCompleted = { ...updated.notCompleted, [ck]: true };
          updated = applyAdaptiveLogic(updated, missionId, false);
          updated = resolveGuessResults(updated, p.name, String(date), missionId, targetMission);
        } else {
          const { finalPts, finalBalance } = calcAwardWithPenalties(updated, p.name, String(date), missionId, rawPts);
          updated.scores        = { ...updated.scores,        [p.name]: (updated.scores?.[p.name]   || 0) + finalPts };
          updated.balances      = { ...updated.balances,      [p.name]: (updated.balances?.[p.name] || 0) + finalBalance };
          updated.verifications = { ...updated.verifications, [`${p.name}:${date}:${missionId}:awarded`]: true };
          updated = applyAdaptiveLogic(updated, missionId, true);
          updated = resolveGuessResults(updated, p.name, String(date), missionId, targetMission);
        }
        changed = true;
      }
...
// Work out what points Player A actually receives after guess penalties
function calcAwardWithPenalties(game, targetName, dateStr, missionId, rawPts) {
  let exactGuessed    = false;
  let categoryGuessed = false;
  Object.keys(game.guesses || {}).forEach(gk => {
    const parts = gk.split(":");
    if (parts.length < 3) return;
    const gDate   = parts[1];
    const gTarget = parts.slice(2).join(":");
    if (gDate !== dateStr || gTarget !== targetName) return;
    const guess = game.guesses[gk];
    if (guess.correct === false) return; // already resolved as wrong, skip
    const targetMission = game.missions.find(m => m.id === missionId);
    if (guess.type === "exact" && guess.missionId === missionId) exactGuessed = true;
    if (guess.type === "category" && targetMission?.category === guess.guess) categoryGuessed = true;
  });
  if (exactGuessed)    return { finalPts: 0,                        finalBalance: 0 };
  if (categoryGuessed) return { finalPts: Math.max(0, rawPts - 1),  finalBalance: Math.max(0, rawPts - 1) };
  return { finalPts: rawPts, finalBalance: rawPts };
}

// Mark guesses as correct/incorrect and award guesser points/balance
function resolveGuessResults(game, targetName, dateStr, missionId, targetMission) {
  let updated = { ...game };
  Object.keys(updated.guesses || {}).forEach(gk => {
    const parts = gk.split(":");
    if (parts.length < 3) return;
    ...
    let correct = false;
    let earnedPts = 0;
    if (guess.type === "category" && targetMission?.category === guess.guess) {
      correct = true; earnedPts = GUESS_CAT_PTS;
    } else if (guess.type === "exact" && guess.missionId === missionId) {
      correct = true; earnedPts = GUESS_EXACT_PTS;
    }
    updated.guesses = { ...updated.guesses, [gk]: { ...guess, correct, awarded: true } };
    if (correct && earnedPts > 0) {
      // Award guesser both score and balance
      updated.scores   = { ...updated.scores,   [guesserName]: (updated.scores?.[guesserName]   || 0) + earnedPts };
      updated.balances = { ...updated.balances,  [guesserName]: (updated.balances?.[guesserName] || 0) + earnedPts };
    }
  });
...
function migrateGame(incoming) {
  ...
  if (!g.missionHistory)   g.missionHistory   = {};
...
// ─────────────────────────────────────────────────────────────
// PLAYER PAGE — TABBED NAV
// ─────────────────────────────────────────────────────────────
function PlayerNav({ active, onNav }) {
  const tabs = [
    { k:"progress",  label:"Today" },
    ...
    <div style={{ display:"flex", gap:0, marginBottom:24, borderBottom:`1px solid rgba(201,168,76,0.15)` }}>
      {tabs.map(({ k, label }) => (
        <button key={k} onClick={() => onNav(k)}
          style={{ fontFamily:"monospace", fontSize:10, letterSpacing:"0.2em", textTransform:"uppercase", padding:"8px 14px", cursor:"pointer", background:"transparent", border:"none", borderBottom:active===k?`2px solid ${C.gold}`:"2px solid transparent", color:active===k?C.gold:C.creamDm, transition:"all 0.2s", marginBottom:-1 }}>
          {label}
        </button>
      ))}
    </div>
...
// ─────────────────────────────────────────────────────────────
function MissionHistoryPanel({ game, viewerName, allPlayers }) {
  const [activeTab, setActiveTab] = useState(viewerName);
  const tabs = allPlayers.map(p => p.name);

  const todayStr = String(todayStamp());
  const isMe = activeTab === viewerName;
  const hist = (game.missionHistory?.[activeTab] || []).filter(entry =>
    // Don't show today's entry for any player until tomorrow
    String(entry.date) !== todayStr
  );

  return (
    <div>
      {allPlayers.length > 1 && (
        <div style={{ display:"flex", gap:0, marginBottom:16, overflowX:"auto", borderBottom:`1px solid rgba(201,168,76,0.15)` }}>
          {tabs.map(name => (
            <button key={name} onClick={() => setActiveTab(name)}
              style={{ fontFamily:"monospace", fontSize:9, letterSpacing:"0.2em", textTransform:"uppercase", padding:"5px 12px", cursor:"pointer", background:"transparent", border:"none", borderBottom:activeTab===name?`2px solid ${C.gold}`:"2px solid transparent", color:activeTab===name?C.gold:C.creamDm, whiteSpace:"nowrap", flexShrink:0, marginBottom:-1 }}>
      ...
      {hist.length === 0 ? (
        <p style={{ fontFamily:"monospace", fontSize:10, color:C.creamDm, lineHeight:1.7, padding:"12px 0" }}>
          {isMe ? "No history yet — complete your first mission." : `No history yet for ${activeTab}.`}
        </p>
      ) : (
        <div style={{ display:"grid", gap:10 }}>
          {hist.map((entry, i) => {
            ...
            const label = new Date(`${y}-${mo}-${d}`).toLocaleDateString("en-GB", { weekday:"short", day:"numeric", month:"short" });
            return (
              <div key={i} style={{ borderLeft:`2px solid ${entry.completed?"rgba(78,158,110,0.5)":"rgba(192,57,43,0.4)"}`, paddingLeft:10 }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:3, flexWrap:"wrap", gap:4 }}>
                  <span style={{ fontFamily:"monospace", fontSize:8, color:C.creamDm, letterSpacing:"0.15em", textTransform:"uppercase" }}>{label}</span>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    <Tag label={entry.category} type="cat"/>
                    {entry.completed && <span style={{ fontFamily:"monospace", fontSize:8, color:"#6ab47a" }}>+{entry.pts}pt{entry.pts!==1?"s":""}</span>}
                  </div>
                </div>
                <p style={{ fontFamily:"'Georgia',serif", fontStyle:"italic", fontSize:11, color:entry.completed?C.cream:C.creamDm, lineHeight:1.55, margin:0, opacity:isMe?1:0.85 }}>
                  {entry.completed ? "✔" : "✖"} {isMe ? entry.mission : (entry.mission.length > 60 ? entry.mission.slice(0,60)+"…" : entry.mission)}
                </p>
              </div>
            );
          })}
...
// GUESS MODAL
// ─────────────────────────────────────────────────────────────
function GuessModal({ guesser, target, game, onSubmit, onClose }) {
  const [guessType,    setGuessType]    = useState("category");
  const [catGuess,     setCatGuess]     = useState("");
  const [exactMission, setExactMission] = useState(null);
  ...
  const categories = ["All", ...[...new Set((game.missions || []).map(m => m.category))].sort()];

  const filteredMissions = (game.missions || []).filter(m => {
    const matchCat  = catFilter === "All" || m.category === catFilter;
    const matchText = !search || m.mission.toLowerCase().includes(search.toLowerCase());
    return matchCat && matchText;
  });

  // Group by category for display
  const grouped = filteredMissions.reduce((acc, m) => {
  ...
  async function submit() {
    if (guessType === "category" && !catGuess) return;
    if (guessType === "exact" && !exactMission) return;
    setConfirmOpen(true);
  }
  ...
  async function confirmSubmit() {
    ...
    setSaving(true);
    if (guessType === "category") {
      await onSubmit(target, "category", catGuess, null);
    } else {
      await onSubmit(target, "exact", exactMission.mission, exactMission.id);
    }
    setSaving(false);
  }

  return (
    <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(11,10,8,0.96)", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      {confirmOpen && (
        <ActionConfirm
          message={`This will cost 1 balance. You currently have ${guesser ? (game.balances?.[guesser.name] || 0) : 0}. Continue?`}
          onYes={confirmSubmit}
          onNo={() => setConfirmOpen(false)}
        />
      )}
      <div style={{ background:C.dark2, border:`1px solid rgba(160,80,200,0.45)`, width:"100%", maxWidth:420, position:"relative", display:"flex", flexDirection:"column", maxHeight:"90vh" }}>
        <div style={{ position:"absolute", top:0, left:0, right:0, height:1, background:`linear-gradient(90deg,transparent,#c070e0,transparent)` }}/>

        {/* Header */}
        <div style={{ padding:"18px 18px 12px", flexShrink:0 }}>
          <p style={{ fontFamily:"monospace", fontSize:9, letterSpacing:"0.4em", textTransform:"uppercase", color:"#c070e0", marginBottom:4 }}>🕵 Guess Mission</p>
          <p style={{ fontFamily:"'Georgia',serif", fontSize:15, color:C.cream, marginBottom:2 }}>What is {target.name} up to?</p>
          <p style={{ fontFamily:"monospace", fontSize:9, color:C.creamDm, lineHeight:1.6 }}>
            Costs 1 balance to submit &nbsp;·&nbsp; Correct category: +{GUESS_CAT_PTS}pt &nbsp;·&nbsp; Exact match: +{GUESS_EXACT_PTS}pts
          </p>
        </div>

        {/* Type toggle */}
        <div style={{ display:"flex", margin:"0 18px 12px", border:`1px solid rgba(160,80,200,0.3)`, flexShrink:0 }}>
          {[["category","Category"],["exact","Exact Mission"]].map(([val,label]) => (
            <button key={val} onClick={() => setGuessType(val)}
              style={{ flex:1, fontFamily:"monospace", fontSize:9, letterSpacing:"0.15em", textTransform:"uppercase", padding:"8px 0", cursor:"pointer", background:guessType===val?"rgba(160,80,200,0.25)":"transparent", color:guessType===val?"#c070e0":C.creamDm, border:"none" }}>
              {label}
            </button>
          ))}
        </div>
          ...
          {guessType === "category" ? (
            <div style={{ display:"grid", gap:6 }}>
              {categories.filter(c => c !== "All").map(cat => (
                <button key={cat} onClick={() => setCatGuess(cat)}
                  style={{ fontFamily:"monospace", fontSize:10, letterSpacing:"0.1em", padding:"9px 12px", cursor:"pointer", textAlign:"left", background:catGuess===cat?"rgba(160,80,200,0.2)":"transparent", color:catGuess===cat?"#c070e0":C.cream, border:`1px solid ${catGuess===cat?"rgba(160,80,200,0.5)":"rgba(201,168,76,0.15)"}` }}>
                  {cat}
                </button>
              ))}
            </div>
            ...
            <div>
              ...
              <div style={{ marginBottom:10, display:"grid", gap:8 }}>
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search missions…"
                  style={{ background:"transparent", border:"none", borderBottom:`1px solid rgba(201,168,76,0.25)`, color:C.cream, fontFamily:"'Georgia',serif", fontSize:13, padding:"5px 4px", outline:"none", width:"100%" }}/>
                <div style={{ display:"flex", gap:4, overflowX:"auto", paddingBottom:2 }}>
                  {categories.map(cat => (
                    <button key={cat} onClick={() => setCatFilter(cat)}
                      style={{ fontFamily:"monospace", fontSize:8, letterSpacing:"0.1em", textTransform:"uppercase", padding:"4px 8px", cursor:"pointer", whiteSpace:"nowrap", flexShrink:0, background:catFilter===cat?"rgba(201,168,76,0.2)":"transparent", color:catFilter===cat?C.gold:C.creamDm, border:`1px solid ${catFilter===cat?"rgba(201,168,76,0.4)":"rgba(201,168,76,0.1)"}` }}>
                      {cat}
                    </button>
                  ))}
                </div>
              ...
              {Object.keys(grouped).sort().map(cat => (
                <div key={cat} style={{ marginBottom:14 }}>
                  <p style={{ fontFamily:"monospace", fontSize:8, letterSpacing:"0.3em", textTransform:"uppercase", color:C.goldDim, marginBottom:6 }}>{cat}</p>
                  <div style={{ display:"grid", gap:5 }}>
                    {grouped[cat].map(m => (
                      <button key={m.id} onClick={() => setExactMission(m)}
                        style={{ fontFamily:"'Georgia',serif", fontStyle:"italic", fontSize:11, padding:"8px 10px", cursor:"pointer", textAlign:"left", lineHeight:1.5, background:exactMission?.id===m.id?"rgba(160,80,200,0.2)":"transparent", color:exactMission?.id===m.id?"#c070e0":C.cream, border:`1px solid ${exactMission?.id===m.id?"rgba(160,80,200,0.5)":"rgba(201,168,76,0.1)"}` }}>
                        {m.mission.replace(/{otherPlayer}/g, target.name)}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {Object.keys(grouped).length === 0 && (
                <p style={{ fontFamily:"monospace", fontSize:10, color:C.creamDm, padding:"8px 0" }}>No missions match.</p>
              )}
            </div>
          )}
        </div>
        ...
        {/* Footer */}
        <div style={{ padding:"12px 18px 18px", flexShrink:0, borderTop:`1px solid rgba(201,168,76,0.1)`, display:"flex", gap:8, marginTop:8 }}>
          <Btn primary sm onClick={submit} disabled={saving || (guessType==="category"?!catGuess:!exactMission)}>
            {saving ? "Submitting…" : "Submit Guess"}
          </Btn>
...
function PlayerView({ player, game, onSaveGame, onSwitchPlayer }) {
  ...
  // Keep status in sync with Firestore — handles cases like an exact guess
  // failing this player's mission while the app is open
  useEffect(() => {
    if (alreadyDone && status !== "done")     setStatus("done");
    if (alreadySkip && status !== "skipped")  setStatus("skipped");
    if (!alreadyDone && !alreadySkip && status !== "pending") setStatus("pending");
  }, [alreadyDone, alreadySkip]);

  const leaderboard = allPlayers.map(p => ({
    ...p,
    score:   game.scores?.[p.name]   || 0,
    balance: game.balances?.[p.name] || 0,
  ...
  const todayScores = allPlayers.map(p => {
    ...
    // Only show as verified if points have been awarded (fully verified)
    const fullyVerified = !!game.verifications?.[`${p.name}:${today}:${pmId}:awarded`];
    return { ...p, done, skip, pm, pck, pmId, fullyVerified };
  });
  ...
  async function execRespond(type) {
    ...
    if (completed) {
      ...
      updated.completions = { ...updated.completions, [ck]: { pts } };
      setStatus("done");
    } else {
      updated.notCompleted = { ...updated.notCompleted, [ck]: true };
      setStatus("skipped");
      updated = applyAdaptiveLogic(updated, mId, false);
    }
    // Now that todayMissions is locked, resolve any pending guesses submitted before the target responded
    const todayStr = String(today);
    let wasFailedByGuess = false;
    Object.keys(updated.guesses || {}).forEach(gk => {
      const parts = gk.split(":");
      if (parts.length < 3) return;
      const gDate   = parts[1];
      const gTarget = parts.slice(2).join(":");
      if (gDate !== todayStr || gTarget !== player.name) return;
      const guess = updated.guesses[gk];
      if (guess.awarded) return; // already resolved

      const guesserName = parts[0];

      if (guess.type === "exact" && guess.missionId === mId) {
        // Exact correct: fail this mission (override the completion we just wrote)
        const newCompletions = { ...updated.completions };
        delete newCompletions[ck];
        updated.completions  = newCompletions;
        updated.notCompleted = { ...updated.notCompleted, [ck]: true };
        if (completed) setStatus("skipped"); // update UI
        wasFailedByGuess = true;
        updated = applyAdaptiveLogic(updated, mId, false);
        // Award guesser the fixed exact-guess reward
        updated.scores   = { ...updated.scores,   [guesserName]: (updated.scores?.[guesserName]   || 0) + GUESS_EXACT_PTS };
        updated.balances = { ...updated.balances,  [guesserName]: (updated.balances?.[guesserName] || 0) + GUESS_EXACT_PTS };
        updated.guesses  = { ...updated.guesses,   [gk]: { ...guess, correct: true, awarded: true } };
      } else if (guess.type === "category" && mission?.category === guess.guess) {
        // Category correct: award guesser, penalty applied at midnight
        updated.scores   = { ...updated.scores,   [guesserName]: (updated.scores?.[guesserName]   || 0) + GUESS_CAT_PTS };
        updated.balances = { ...updated.balances,  [guesserName]: (updated.balances?.[guesserName] || 0) + GUESS_CAT_PTS };
        updated.guesses  = { ...updated.guesses,   [gk]: { ...guess, correct: true, awarded: true } };
      } else {
        // Wrong guess
        updated.guesses = { ...updated.guesses, [gk]: { ...guess, correct: false, awarded: true } };
      }
    });

    // Write mission history AFTER guess resolution so completed flag is accurate
    const finalCompleted = completed && !wasFailedByGuess;
    const histEntry = {
      date:      String(today),
      missionId: mId,
      ...
      pts:       finalCompleted ? (DIFF_SCORE[mission?.difficulty] || 1) : 0,
    };
    const prevHist = updated.missionHistory?.[player.name] || [];
    updated.missionHistory = {
      ...updated.missionHistory,
      [player.name]: [histEntry, ...prevHist].slice(0, HISTORY_LIMIT),
    };
  ...
  function respond(type) {
    ...
    const message = type === "done"
      ? `Mark mission as completed? You'll earn ${pts} point${pts !== 1 ? "s" : ""} automatically at midnight.`
      : "Mark mission as not completed?";
    setPendingAction({ message, exec: async () => execRespond(type) });
  }
  ...
  async function execRate(val) {
    ...
    setRating(newRating);
    let updated = { ...gameRef.current, ratings: { ...gameRef.current.ratings } };
  ...
  function rate(val) {
    if (!mId) return;
    if (val === "dislike") {
      setPendingAction({ message:`Vote to remove this mission? You won't receive it again, and if ${BAD_VOTE_THRESHOLD} players vote bad it will be deleted.`, exec: async () => execRate(val) });
    } else {
      execRate(val);
    }
  }

  async function submitGuess(targetPlayer, guessType, guessValue, guessMissionId) {
    const gk = `${player.name}:${today}:${targetPlayer.name}`;
    if (gameRef.current.guesses?.[gk]) return;

    // Deduct 1 balance from guesser (not score)
    const entry = { type: guessType, guess: guessValue, missionId: guessMissionId || null, awarded: false, correct: null };
    let updated = {
      ...gameRef.current,
      guesses:  { ...gameRef.current.guesses,  [gk]: entry },
      balances: { ...gameRef.current.balances, [player.name]: Math.max(0, (gameRef.current.balances?.[player.name] || 0) - 1) },
    };

    const targetMissionId  = gameRef.current.todayMissions?.[`${targetPlayer.name}:${today}`];
    const targetCk         = `${targetPlayer.name}:${today}:${targetMissionId}`;
    const targetMission    = targetMissionId ? gameRef.current.missions.find(m => m.id === targetMissionId) : null;
    const alreadyCompleted = targetMissionId && !!gameRef.current.completions?.[targetCk];
    const alreadyAwarded   = targetMissionId && !!gameRef.current.verifications?.[`${targetPlayer.name}:${today}:${targetMissionId}:awarded`];

    // Determine if this guess is correct
    const isExactCorrect    = guessType === "exact"    && guessMissionId === targetMissionId;
    const isCategoryCorrect = guessType === "category" && targetMission?.category === guessValue;

    // ── EXACT CORRECT: fail the mission for target, award guesser ──
    if (isExactCorrect && targetMissionId) {
      const completionRecord = gameRef.current.completions?.[targetCk];
      const rawPts = (typeof completionRecord === "object" ? completionRecord?.pts : null)
                     || DIFF_SCORE[targetMission?.difficulty] || 1;

      // Flip target to not completed regardless of current status
      const newCompletions = { ...updated.completions };
      delete newCompletions[targetCk];
      updated.completions  = newCompletions;
      updated.notCompleted = { ...updated.notCompleted, [targetCk]: true };

      // If target was already awarded points, claw them back
      if (alreadyAwarded) {
        updated.scores   = { ...updated.scores,   [targetPlayer.name]: Math.max(0, (updated.scores?.[targetPlayer.name]   || 0) - rawPts) };
        updated.balances = { ...updated.balances,  [targetPlayer.name]: Math.max(0, (updated.balances?.[targetPlayer.name] || 0) - rawPts) };
        // Remove awarded flag so auto-verify doesn't re-award
        const newVerifs = { ...updated.verifications };
        delete newVerifs[`${targetPlayer.name}:${today}:${targetMissionId}:awarded`];
        updated.verifications = newVerifs;
      }

      // Increase difficulty for target's mission (same as a normal failure)
      updated = applyAdaptiveLogic(updated, targetMissionId, false);

      // Award guesser the fixed exact-guess reward (separate from rawPts which is for clawback only)
      updated.scores   = { ...updated.scores,   [player.name]: (updated.scores?.[player.name]   || 0) + GUESS_EXACT_PTS };
      updated.balances = { ...updated.balances,  [player.name]: (updated.balances?.[player.name] || 0) + GUESS_EXACT_PTS };

      // Mark guess resolved
      updated.guesses = { ...updated.guesses, [gk]: { ...entry, correct: true, awarded: true } };

    // ── CATEGORY CORRECT: apply -1 penalty to target's award ──
    } else if (isCategoryCorrect && targetMissionId) {
      updated.guesses = { ...updated.guesses, [gk]: { ...entry, correct: true, awarded: true } };
      // Award guesser both score and balance
      updated.scores   = { ...updated.scores,   [player.name]: (updated.scores?.[player.name]   || 0) + GUESS_CAT_PTS };
      updated.balances = { ...updated.balances,  [player.name]: (updated.balances?.[player.name] || 0) + GUESS_CAT_PTS };

      if (alreadyCompleted && !alreadyAwarded) {
        // Award target with -1 penalty right now
        const completionRecord = gameRef.current.completions?.[targetCk];
        const rawPts = (typeof completionRecord === "object" ? completionRecord?.pts : null)
                       || DIFF_SCORE[targetMission?.difficulty] || 1;
        const penalised = Math.max(0, rawPts - 1);
        updated.scores   = { ...updated.scores,   [targetPlayer.name]: (updated.scores?.[targetPlayer.name]   || 0) + penalised };
        updated.balances = { ...updated.balances,  [targetPlayer.name]: (updated.balances?.[targetPlayer.name] || 0) + penalised };
        updated.verifications = { ...updated.verifications, [`${targetPlayer.name}:${today}:${targetMissionId}:awarded`]: true };
        updated = applyAdaptiveLogic(updated, targetMissionId, true);
      }
      // If already awarded or not yet completed — penalty applied at midnight via calcAwardWithPenalties

    // ── WRONG GUESS: only mark resolved if we know the target's mission ──
    // If targetMissionId is unknown, leave awarded:false — midnight will resolve it
    } else if (targetMissionId) {
      updated.guesses = { ...updated.guesses, [gk]: { ...entry, correct: false, awarded: true } };
    }
    // targetMissionId unknown → stays awarded:false, resolved at midnight
  ...
  const myScore   = game.scores?.[player.name]   || 0;
  const myBalance = game.balances?.[player.name] || 0;
  ...
  return (
    <>
      ...
      <PageWrap narrow>
        ...
        <div style={{ textAlign:"center", marginBottom:20 }}>
          ...
          <div style={{ display:"flex", gap:18, justifyContent:"center", marginBottom:16 }}>
            ...
            <div style={{ textAlign:"center" }}>
              ...
              <div style={{ fontFamily:"'Georgia',serif", fontSize:22, color:"#6ab47a", fontWeight:700 }}>{myBalance}</div>
            </div>
          </div>
        </div>

        {/* Mission card — always visible */}
        <div style={{ textAlign:"center", marginBottom:24, background:C.dark2, border:`1px solid rgba(201,168,76,0.2)`, padding:"20px 16px", position:"relative" }}>
          <div style={{ position:"absolute", top:0, left:0, right:0, height:1, background:`linear-gradient(90deg,transparent,${C.gold},transparent)` }}/>
          ...
          {mission ? (
            <>
              ...
              {status === "done" && (() => {
                const pointsAwarded = !!game.verifications?.[`${player.name}:${today}:${mId}:awarded`];
                const completionRecord = game.completions?.[ck];
                const awardedPts = (typeof completionRecord === "object" ? completionRecord?.pts : null) || score;
                return (
                  <div style={{ marginTop:8 }}>
                    ...
                    <div style={{ fontFamily:"monospace", fontSize:10, color:pointsAwarded?"#6ab47a":C.creamDm, marginTop:8, letterSpacing:"0.1em" }}>
                      {pointsAwarded ? `+${awardedPts} pt${awardedPts !== 1 ? "s" : ""} awarded ✓` : "Points awarded automatically at midnight."}
                    </div>
                  </div>
                );
              })()}
        ...
        {allPlayers.length > 0 && (
          <>
            <PlayerNav active={activeTab} onNav={setActiveTab}/>
            ...
            {activeTab === "progress" && (
              <div>
                {todayScores.map(p => {
                  ...
                  const gk = `${player.name}:${today}:${p.name}`;
                  const myGuess = game.guesses?.[gk];
                  const canGuess = myBalance >= 1;
                  return (
                    <div key={p.id} style={{ background:C.dark2, border:`1px solid rgba(201,168,76,0.1)`, padding:"10px 14px", marginBottom:6 }}>
                      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:(!isMe && myGuess) ? 6 : 0 }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                          {myGuess ? (
                            <span style={{ fontFamily:"monospace", fontSize:9, letterSpacing:"0.12em", textTransform:"uppercase",
                              color: myGuess.awarded ? (myGuess.correct?"#6ab47a":"#e05a4b") : "#c070e0" }}>
                              {myGuess.awarded
                                ? (myGuess.correct ? `🕵 +${myGuess.type==="exact"?GUESS_EXACT_PTS:GUESS_CAT_PTS}pt correct!` : "🕵 Wrong guess")
                                : `🕵 ${myGuess.type==="category"?"Cat: ":"Exact: "}${myGuess.guess.length>20?myGuess.guess.slice(0,20)+"…":myGuess.guess}`}
                            </span>
                          ) : (
                            <button onClick={() => canGuess && setGuessTarget(p)} disabled={!canGuess}
                              style={{ fontFamily:"monospace", fontSize:9, letterSpacing:"0.15em", textTransform:"uppercase", padding:"4px 12px", background:"transparent", border:`1px solid ${canGuess?"rgba(160,80,200,0.4)":"rgba(160,80,200,0.15)"}`, color:canGuess?"#c070e0":"rgba(192,112,224,0.3)", cursor:canGuess?"pointer":"not-allowed" }}>
                              🕵 Guess {canGuess ? "(costs 1)" : "(no balance)"}
                            </button>
                          )}
                        </div>
                      )}
                ...
                {leaderboard.map((p, i) => (
                  <div key={p.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"8px 14px", marginBottom:6, background:C.dark2, border:`1px solid rgba(201,168,76,0.1)` }}>
                    ...
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontFamily:"monospace", fontSize:12, color:C.gold, fontWeight:700 }}>{p.score} pts</div>
                      <div style={{ fontFamily:"monospace", fontSize:9, color:"#6ab47a", opacity:0.8 }}>💰 {p.balance} balance</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* HISTORY TAB */}
            {activeTab === "history" && (
              <MissionHistoryPanel game={game} viewerName={player.name} allPlayers={allPlayers}/>
            )}
          </>
        )}
...
function PlayersTab({ game, onSave }) {
  ...
  const gameRef = useRef(game);
  useEffect(() => { gameRef.current = game; }, [game]);
  // Resync local players list if it changes externally (e.g. another device adds a player)
  useEffect(() => {
  ...
  async function save() { setSaving(true); await onSave({ ...gameRef.current, players }); setSaving(false); }

  async function clearPassword(playerId) {
    const g = gameRef.current;
    const newPw = { ...g.passwords }; delete newPw[playerId];
    const newLO = { ...g.lockedOut, [playerId]: true };
    await onSave({ ...g, players, passwords:newPw, lockedOut:newLO });
    setConfirm(null);
  }

  async function resetMission(player) {
    setConfirm(null); // always close immediately regardless of outcome
    const g      = gameRef.current;
    const today  = todayStamp();
    const current = missionForPlayer(player.name, today, g.missions, g.missionOverrides, g.exclusions?.[player.name]);
    if (!current) return;
    const pool = g.missions.filter(m => m.id !== current.id);
    ...
    const nc         = { ...g.notCompleted }; delete nc[oldCk];
    const co         = { ...g.completions  }; delete co[oldCk];
    const overrides  = { ...g.missionOverrides, [`${player.name}:${today}`]: newMission.id };
    await onSave({ ...g, players, completions:co, notCompleted:nc, missionOverrides:overrides });
  }
  ...
  return (
    <div>
      ...
      {confirm?.type === "resetMission" && (
        <Confirm title="Reset Today's Mission" text={`Reset ${confirm.player.name}'s mission response for today.`}
...
function MissionsTab({ game, onSave }) {
  ...
  const atRiskMissions  = missions.filter(m => { const bv = badVoteCount(m.id); return bv >= BAD_VOTE_THRESHOLD - 1 && bv < BAD_VOTE_THRESHOLD; }).length;
  ...
  async function saveAll() { setSaving(true); await onSave({ ...gameRef.current, missions }); setSaving(false); }
  ...
  function handleImport(e) {
    ...
    reader.onload = ev => {
      try {
        ...
        if (invalid.length) throw new Error(`${invalid.length} mission(s) missing required fields.`);
        const badDiff = parsed.filter(m => !DIFFICULTIES.includes(m.difficulty));
        if (badDiff.length) throw new Error(`Invalid difficulty in ${badDiff.length} mission(s). Must be: ${DIFFICULTIES.join(", ")}.`);
        const existingIds = new Set(missions.map(m => m.id));
        let added = 0, skipped = 0;
        const toAdd = [];
        parsed.forEach(m => {
          if (m.id && existingIds.has(m.id)) { skipped++; return; }
          toAdd.push({ id: m.id || "m"+uid(), mission:m.mission, difficulty:m.difficulty, category:m.category });
          added++;
        });
  ...
  return (
    <div>
      ...
      {confirmDeleteAll && (
        <Confirm title="Delete All Missions" text={`Permanently delete all ${missions.length} missions. Cannot be undone.`}
          ...
          onCancel={() => setConfirmDeleteAll(false)} label="Delete All"/>
      )}
      {importStatus && (
        <div style={{ fontFamily:"monospace", fontSize:10, letterSpacing:"0.1em", padding:"8px 14px", marginBottom:14,
          ...
          color:importStatus.type==="success"?"#6ab47a":"#e05a4b", display:"flex", alignItems:"center", justifyContent:"space-between", gap:10 }}>
      ...
      {editing && (
        <Card style={{ marginBottom:16, border:`1px solid rgba(201,168,76,0.45)` }}>
          ...
          <textarea value={form.mission} onChange={e => setForm(f => ({ ...f, mission:e.target.value }))} placeholder="Use {otherPlayer} as a placeholder. e.g. 'Give {otherPlayer} a high five…'" rows={3}
      ...
      {totalBadVotes > 0 && (
        <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:14 }}>
          <div style={{ fontFamily:"monospace", fontSize:10, padding:"7px 14px", background:"rgba(192,57,43,0.08)", border:`1px solid rgba(192,57,43,0.25)`, color:"#e05a4b" }}>
            👎 {totalBadVotes} bad vote{totalBadVotes!==1?"s":""} total
          </div>
          {atRiskMissions > 0 && <div style={{ fontFamily:"monospace", fontSize:10, padding:"7px 14px", background:"rgba(201,168,76,0.07)", border:`1px solid rgba(201,168,76,0.25)`, color:C.gold }}>⚠ {atRiskMissions} close to deletion</div>}
          {willDeleteCount > 0 && <div style={{ fontFamily:"monospace", fontSize:10, padding:"7px 14px", background:"rgba(192,57,43,0.12)", border:`1px solid rgba(192,57,43,0.5)`, color:"#e05a4b" }}>🗑 {willDeleteCount} queued for deletion</div>}
        </div>
      )}
            ...
            <div key={m.id} style={{ background:C.dark2, border:`1px solid ${bv>=BAD_VOTE_THRESHOLD-1?"rgba(192,57,43,0.3)":"rgba(201,168,76,0.1)"}`, padding:"10px 12px", marginBottom:7, display:"flex", gap:10, alignItems:"flex-start" }}>
              <div style={{ flex:1 }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}>
                  ...
                  {bv>0 && <span style={{ fontFamily:"monospace", fontSize:9, color:bv>=BAD_VOTE_THRESHOLD?"#e05a4b":C.creamDm, background:"rgba(192,57,43,0.1)", border:`1px solid rgba(192,57,43,0.25)`, padding:"2px 7px" }}>
...
function ScoreboardTab({ game, onSave }) {
  ...
  const [saving,   setSaving]   = useState(false);
  const [editing,  setEditing]  = useState(null);
  const [editVals, setEditVals] = useState({ score: "", balance: "" });
  const gameRef = useRef(game);
  useEffect(() => { gameRef.current = game; }, [game]);

  const scores = (game.players || []).map(p => {
    const score     = game.scores?.[p.name]   || 0;
    const balance   = game.balances?.[p.name] || 0;
    const mission   = missionForPlayer(p.name, today, game.missions, game.missionOverrides, game.exclusions?.[p.name]);
    const ck        = `${p.name}:${today}:${mission?.id}`;
    const doneToday = !!game.completions?.[ck];
    const skipToday = !!game.notCompleted?.[ck];
    return { ...p, score, balance, doneToday, skipToday };
  }).sort((a, b) => b.score - a.score);
  ...
  function startEdit(p) {
    setEditing(p.name);
    setEditVals({ score: String(p.score), balance: String(p.balance) });
  }

  async function saveEdit(playerName) {
    const newScore   = parseInt(editVals.score,   10);
    const newBalance = parseInt(editVals.balance, 10);
    if (isNaN(newScore) || isNaN(newBalance)) return;
    setSaving(true);
    const latest = gameRef.current;
    await onSave({
      ...latest,
      scores:   { ...latest.scores,   [playerName]: newScore   },
      balances: { ...latest.balances, [playerName]: newBalance },
    });
    setSaving(false);
    setEditing(null);
  }
  ...
  async function reset() {
    setSaving(true);
    const latest = gameRef.current;
    await onSave({ ...latest, completions:{}, notCompleted:{}, ratings:{}, balances:{}, scores:{}, verifications:{}, exclusions:{}, todayMissions:{}, guesses:{}, missionHistory:{} });
    setSaving(false);
    setConfirm(false);
  }
          ...
          <div style={{ display:"flex", alignItems:"center", gap:14 }}>
              ...
              {editing === p.name ? (
                <div style={{ display:"grid", gap:8 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <label style={{ fontFamily:"monospace", fontSize:9, letterSpacing:"0.2em", textTransform:"uppercase", color:C.gold, width:56, flexShrink:0 }}>Score</label>
                    <input type="number" value={editVals.score} onChange={e => setEditVals(v => ({ ...v, score: e.target.value }))}
                      style={{ width:70, background:"transparent", border:"none", borderBottom:`1px solid rgba(201,168,76,0.4)`, color:C.cream, fontFamily:"monospace", fontSize:13, padding:"3px 4px", outline:"none", textAlign:"right" }}/>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <label style={{ fontFamily:"monospace", fontSize:9, letterSpacing:"0.2em", textTransform:"uppercase", color:"#6ab47a", width:56, flexShrink:0 }}>Balance</label>
                    <input type="number" value={editVals.balance} onChange={e => setEditVals(v => ({ ...v, balance: e.target.value }))}
                      style={{ width:70, background:"transparent", border:"none", borderBottom:`1px solid rgba(106,180,122,0.4)`, color:C.cream, fontFamily:"monospace", fontSize:13, padding:"3px 4px", outline:"none", textAlign:"right" }}/>
                  </div>
                  <div style={{ display:"flex", gap:6, marginTop:2 }}>
                    <Btn primary sm onClick={() => saveEdit(p.name)} disabled={saving}>Save</Btn>
                    <Btn ghost sm onClick={() => setEditing(null)}>Cancel</Btn>
                  </div>
                </div>
              ) : (
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  <div style={{ fontFamily:"monospace", fontSize:10, color:C.creamDm }}>💰 {p.balance} balance</div>
                  <button onClick={() => startEdit(p)}
                    style={{ fontFamily:"monospace", fontSize:9, letterSpacing:"0.15em", textTransform:"uppercase", padding:"3px 10px", background:"transparent", border:`1px solid rgba(201,168,76,0.25)`, color:C.creamDm, cursor:"pointer" }}>
                    ✎ Edit
                  </button>
                </div>
              )}
            ...
            {editing !== p.name && (
              <div style={{ textAlign:"right", flexShrink:0 }}>
                <div style={{ fontFamily:"'Georgia',serif", fontSize:26, color:C.gold, fontWeight:700 }}>{p.score}</div>
                <div style={{ fontFamily:"monospace", fontSize:9, color:C.creamDm }}>pts</div>
              </div>
            )}
          </div>
        </Card>
...
export default function App() {
  ...
  const savingExpire = useRef(false);

  useEffect(() => {
    const ident = loadIdentity();

    const unsub = subscribeShared(incoming => {
      // Guard: never replace real game data with defaultGame if incoming is null/undefined
      // (can happen transiently during Firestore writes — treat as a no-op)
      if (!incoming) return;

      let g = migrateGame(incoming);

      // Auto-expire missions from previous days — write back only once, not re-entrantly
      if (!savingExpire.current) {
        const expired = autoExpireMissions(g);
        if (expired !== g) {
          g = expired;
          savingExpire.current = true;
          saveMissions(g.missions)
            .then(() => saveShared(g))
            .finally(() => { savingExpire.current = false; });
        }
      }
    ...
    // On first load, pull shared missions — only apply if subscribeShared hasn't fired yet
    // (prevents loadMissions overwriting a newer version already set by the snapshot)
    loadMissions().then(sharedMissions => {
      if (sharedMissions && firstLoad.current) {
        setGame(prev => prev ? { ...prev, missions: sharedMissions } : null);
      }
    });

    // Seed Firestore only if the document genuinely doesn't exist yet
    loadShared().then(g => {
      if (!g) {
        const fresh = defaultGame();
        saveShared(fresh);
        saveMissions(fresh.missions);
      }
    });
  ...
  async function saveGame(g) {
    setGame(g);
    // Save missions to shared key first, then game state
    // This ensures subscribeShared sees up-to-date missions
  ...
  function selectPlayer(player) {
    ...
    saveIdentity(player);
    const g = gameRef.current;
    const lockedOut = !!g.lockedOut?.[player.id];
    const hasPw     = !!g.passwords?.[player.id];
  ...
  async function setPassword(pw) {
    ...
    const g       = gameRef.current;
    const newLO   = { ...g.lockedOut }; delete newLO[identity.id];
    const newGame = { ...g, passwords:{ ...g.passwords, [identity.id]:hashed }, lockedOut:newLO };
