...
function PlayerView({ player, game, onSaveGame, onSwitchPlayer }) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      if (fresh) {
        const migrated = migrateGame(fresh);
  ...
  async function execRespond(type) {
    ...
    setSaving(true);
    let updated = { ...game };
    // Store the missionId so other clients can reliably look it up
...
export default function App() {
  ...
  useEffect(() => {
    Promise.all([loadShared(), loadIdentity(), loadMissions()]).then(([g, ident, sharedMissions]) => {
      ...
      // Always use shared missions as source of truth; seed from DEFAULT_MISSIONS if none exist yet
      if (sharedMissions) {
        game.missions = sharedMissions;
      } else {
        // First time — seed shared missions from defaults and save
