...
// ─────────────────────────────────────────────────────────────
// GUESS SCORING HELPER
// ─────────────────────────────────────────────────────────────
function awardGuessPoints(game, targetName, dateStamp, missionId, targetMission) {
  let updated = { ...game };
  const dateStr = String(dateStamp);
      ...
      <div style={{ background:C.dark2, border:`1px solid rgba(160,80,200,0.45)`, width:"100%", maxWidth:420, position:"relative", display:"flex", flexDirection:"column", maxHeight:"90vh" }}>
        ...
        <div style={{ padding:"18px 18px 12px", flexShrink:0 }}>
          ...
          <p style={{ fontFamily:"monospace", fontSize:9, color:C.creamDm, lineHeight:1.6 }}>
            Category: +{GUESS_CAT_PTS}pt &nbsp;·&nbsp; Exact match: +{GUESS_EXACT_PTS}pts
          </p>
        </div>
...
function PlayerView({ player, game, onSaveGame, onSwitchPlayer }) {
  ...
  async function verifyPlayer(targetPlayer, pmId) {
    ...
    const alreadyAwarded = !!game.verifications?.[`${targetPlayer.name}:${today}:${pmId}:awarded`];
    let updated = { ...game, verifications: { ...game.verifications, [vk]: true } };
    if (!alreadyAwarded) {
      const targetCk = `${targetPlayer.name}:${today}:${pmId}`;
      const completionRecord = game.completions?.[targetCk];
      const targetMission = game.missions.find(m => m.id === pmId);
      const pts = (typeof completionRecord === "object" ? completionRecord?.pts : null)
                  || DIFF_SCORE[targetMission?.difficulty] || 1;
      updated.balances = { ...updated.balances, [targetPlayer.name]: (updated.balances?.[targetPlayer.name] || 0) + pts };
      updated.verifications = { ...updated.verifications, [`${targetPlayer.name}:${today}:${pmId}:awarded`]: true };
      updated = applyAdaptiveLogic(updated, pmId, true);
      // Award guess points to anyone who guessed this player's mission correctly
      updated = awardGuessPoints(updated, targetPlayer.name, today, pmId, targetMission);
    }
    await onSaveGame(updated);
  }
  ...
  async function submitGuess(targetPlayer, guessType, guessValue, guessMissionId) {
    ...
    const updated = { ...game, guesses: { ...game.guesses, [gk]: entry } };
    await onSaveGame(updated);
    setGuessTarget(null);
  }
                ...
                return (
                  <div style={{ marginTop:8 }}>
                    ...
                    <div style={{ fontFamily:"monospace", fontSize:10, color:pointsAwarded?"#6ab47a":C.creamDm, marginTop:8, letterSpacing:"0.1em" }}>
                      {pointsAwarded ? `+${awardedPts} pt${awardedPts !== 1 ? "s" : ""} awarded ✓` : "Waiting for another player to verify…"}
                    </div>
                  </div>
                );
              })()}
            ...
            {activeTab === "progress" && (
              <div>
                {todayScores.map(p => {
                  ...
                  // Guess button visible only until mission is fully verified
                  const guessOpen = !p.fullyVerified;
                  return (
                    <div key={p.id} style={{ background:C.dark2, border:`1px solid rgba(201,168,76,0.1)`, padding:"10px 14px", marginBottom:6 }}>
                      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:(!isMe && (p.done || myGuess)) ? 6 : 0 }}>
                        ...
                        <span style={{ fontFamily:"monospace", fontSize:10, color:p.done?"#6ab47a":p.skip?"#e05a4b":C.creamDm }}>
                          {p.done
                            ? (p.fullyVerified ? "✓ Verified" : "✓ Done — awaiting verify")
                            : p.skip ? "✗ Not done" : "Pending"}
                        </span>
                      </div>
                      {!isMe && (
                        <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                          {/* Verify button — only shown when done AND not yet fully verified */}
                          {p.done && !p.fullyVerified && (
                            p.iVerified ? (
                              <span style={{ fontFamily:"monospace", fontSize:9, color:"#6ab47a", letterSpacing:"0.15em", textTransform:"uppercase" }}>✓ You verified</span>
                            ) : (
                              <button onClick={() => verifyPlayer(p, p.pmId)}
                                style={{ fontFamily:"monospace", fontSize:9, letterSpacing:"0.15em", textTransform:"uppercase", padding:"4px 12px", background:"transparent", border:`1px solid rgba(201,168,76,0.35)`, color:C.gold, cursor:"pointer" }}>
                                👁 Verify
                              </button>
                            )
                          )}
                          {/* Guess button — hidden once fully verified */}
                          {guessOpen && (
                            myGuess ? (
                              ...
                              <button onClick={() => setGuessTarget(p)}
                                style={{ fontFamily:"monospace", fontSize:9, letterSpacing:"0.15em", textTransform:"uppercase", padding:"4px 12px", background:"transparent", border:`1px solid rgba(160,80,200,0.4)`, color:"#c070e0", cursor:"pointer" }}>
                                🕵 Guess Mission
                              </button>
                            )
                          )}
...
function ScoreboardTab({ game, onSave }) {
  ...
  async function reset() {
    setSaving(true);
    await onSave({ ...game, completions:{}, notCompleted:{}, ratings:{}, balances:{}, verifications:{}, exclusions:{}, todayMissions:{} });
    setSaving(false);
    setConfirm(false);
  }
